(() => {
  ...
  const premiumAnimations = {
    playLevelTransition: () => {
      ...
      transition.innerHTML = `
        <div style="color: white; font-size: 2rem; font-weight: bold; text-align: center;">
          Level ${currentLevel}
        </div>
      `;
    ...
    enhancedCelebrateWin: (color) => {
      // Tile cascade animation
      grid.flat().forEach((cell, index) => {
        setTimeout(() => {
          cell.el.style.animation = "premiumFlash 500ms cubic-bezier(0.34, 1.56, 0.64, 1) forwards";
      ...
      // Enhanced particles
      premiumAnimations.spawnEnhancedParticles(color);
      
      // Board success glow
      boardEl.classList.add('success-glow');
      setTimeout(() => {
        boardEl.classList.remove('success-glow');
      }, 2000);
    },

    spawnEnhancedParticles: (color) => {
      const particles = 24;
    ...
    smoothColorTransition: (element, newColor) => {
      element.style.transition = 'background-color 180ms cubic-bezier(0.25, 0.46, 0.45, 0.94)';
  ...
  const setupPremiumInteractions = () => {
    const buttons = document.querySelectorAll('.color-btn, .icon-btn, .reset-btn, .ghost-btn');
  ...
  const calculateMoves = (size, level) => {
    if (level <= 3) {
      ...
      return Math.round(size * 2.0);  
    } else if (level <= 15) {
      return Math.round(size * 1.8);
    } else {
      return Math.round(size * 1.7);  // The perfect fix!
    }
  };

  const loadLevel = (level, isReset = false) => {
    if (!isReset && level > currentLevel) {
      premiumAnimations.playLevelTransition();
    }
    ...
    steps.forEach((tiles, idx) => {
      setTimeout(() => {
        tiles.forEach(([r, c]) => {
          const cell = grid[r][c];
          cell.color = newColor;
          premiumAnimations.smoothColorTransition(cell.el, newColor);
  ...
  const checkState = () => {
    ...
    if (victory) {
      ...
      premiumAnimations.enhancedCelebrateWin(activeColor);
